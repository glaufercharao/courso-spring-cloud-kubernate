package org.gsamtechnology.springcloud.msvc.cursos.resources;
...
public class CursoController {
    ...
    public ResponseEntity<?> findById(@PathVariable Long id) {
        Optional<Curso> o = service.findUsuarioById(id);//service.porId(id);
        if (o.isPresent()) {
    ...
    public ResponseEntity<?> createUsuario(@RequestBody Usuario usuario, @PathVariable Long cursoId) {
        Optional<Usuario> o;
        try {
            o = service.createUsuario(usuario, cursoId);
        } catch (FeignException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
    ...
    public ResponseEntity<?> removeUsuario(@RequestBody Usuario usuario, @PathVariable Long cursoId) {
        Optional<Usuario> o;
        try {
            o = service.removeUsuario(usuario, cursoId);
        } catch (FeignException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(Collections.singletonMap("mensaje", "No existe el usuario por " +
                            "el id o error en la comunicacion: " + e.getMessage()));
